...
function renderTablePage(pageNumber, data) {
    ...
    paginatedData.forEach(row => {
        ...
        navigateBtn.textContent = 'Details';
        navigateBtn.className = 'button';
        navigateBtn.style.backgroundColor = 'purple';
        navigateBtn.addEventListener('click', function () {
            event.preventDefault();
            window.location.href = "/vmpedia/vmscreen.aspx" + "?id=" + row[idNum] + "&vc=" + row[vcNum];
        });

        const secondBtn = document.createElement('button');
        secondBtn.textContent = 'Action';
        secondBtn.className = 'button';
        secondBtn.style.backgroundColor = 'orange';
        secondBtn.addEventListener('click', function () {
            event.preventDefault();
            window.location.href = "https://smart/Flow/Dynamic/Start?FlowKey=364372_Update_2";
        });
        actionCell.appendChild(navigateBtn);
        actionCell.appendChild(secondBtn);
